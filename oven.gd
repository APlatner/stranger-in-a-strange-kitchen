...
@onready var recipes: Array[Recipe] = [
		Recipe.new("Cake", ["Cake Batter"]),
		Recipe.new("Garlic Bread", ["Bread", "Butter", "Garlic"]),
		Recipe.new("Pizza", ["Pizza Crust", "Tomato Sauce", "Cheese"]),
		Recipe.new("Pie", ["Pie Crust", "Berries", "Sugar"]),
	]
...
@onready var off_oven: CompressedTexture2D = preload("res://oven_off.png") as CompressedTexture2D
@onready var on_oven: CompressedTexture2D = preload("res://oven_on.png") as CompressedTexture2D
...
func can_activate(player: Player) -> bool:
	if player.item_name != "":
		...
		for recipe in current_recipes:
			if recipe.ingredients.find(player.item_name) > -1:
				Globals.set_prompt_display.emit("Press [Space] to add " + player.item_name + " to oven.")
				return true
		return false
	else:
		return false
