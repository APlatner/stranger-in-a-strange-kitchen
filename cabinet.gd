class_name Cabinet extends Activatable


@onready var ingredients: Array[String] = ["Tomato", "Potato", "Flour", "Sugar", "Garlic", "Bread", "Yeast"]
...
func can_activate(player: Player) -> bool:
	if player.item_name == "":
		Globals.set_prompt_display.emit("Press [Space] to open cabinet.")
		return true
	else:
		return false
