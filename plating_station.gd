...
@onready var recipes: Array[Recipe] = [
	Recipe.new("Fish Dinner", ["Fish", "Baked Potato"]),
	Recipe.new("Steak Dinner", ["Steak", "Mashed Potato", "Mushrooms"]),
	Recipe.new("Birthday Cake", ["Cake", "Frosting"]),
	Recipe.new("Salad", ["Lettuce", "Tomato"]),
]
...
func can_activate(player: Player) -> bool:
	if player.item_name != "":
		...
		for recipe in current_recipes:
			if recipe.ingredients.find(player.item_name) > -1:
				Globals.set_prompt_display.emit("Press [Space] to add " + player.item_name + " to plate.")
				return true
		return false
	else:
		return false
